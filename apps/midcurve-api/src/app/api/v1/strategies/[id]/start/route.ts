/**
 * Strategy Start Operation Polling Endpoint
 *
 * GET /api/v1/strategies/:id/start
 *
 * Authentication: Required (session only)
 *
 * Proxies to the EVM service to poll start operation status.
 * Used for polling after initiating a start lifecycle operation.
 *
 ...
 * }
 *
 * Returns 404 if no start operation found.
 */
...
/**
 * OPTIONS /api/v1/strategies/:id/start
 *
 ...
 * GET /api/v1/strategies/:id/start
 *
 * Poll the start operation status.
 */
    ...
    try {
      ...
      try {
        const evmResponse = await fetch(
          `${evmServiceUrl}/api/strategy/${contractAddress}/start`,
          {
            method: 'GET',
      ...
      const errorResponse = createErrorResponse(
        ApiErrorCode.INTERNAL_SERVER_ERROR,
        'Failed to poll start operation status',
