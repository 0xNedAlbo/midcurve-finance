...
node packages/midcurve-database/prisma/dist/seed-contracts.cjs

# 3. Seed chart of accounts (idempotent)
echo "Seeding chart of accounts..."
node packages/midcurve-database/prisma/dist/seed-accounts.cjs

# 4. Seed CoinGecko tokens (idempotent)
echo "Seeding CoinGecko tokens..."
node packages/midcurve-database/prisma/dist/seed-coingecko-tokens.cjs
