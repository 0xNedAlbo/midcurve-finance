...
export function ConfigureStep() {
  ...
  // Cast vault-specific config and state
  const vaultConfig = useMemo(() => {
    if (!position) return null;
    return position.config as UniswapV3VaultPositionConfigResponse;
  }, [position]);

  const vaultState = useMemo(() => {
    if (!position) return null;
    return position.state as UniswapV3VaultPositionStateResponse;
  }, [position]);
  ...
  // Extract position data with proportional user liquidity
  const positionConfig = useMemo(() => {
    if (!vaultConfig || !vaultState) return null;
    ...
    // User's proportional liquidity: vaultLiquidity * sharesBalance / totalSupply
    const userLiquidity =
      totalSupply > 0n
        ? (vaultLiquidity * sharesBalance) / totalSupply
        : 0n;
    return {
      tickLower: vaultConfig.tickLower,
      tickUpper: vaultConfig.tickUpper,
