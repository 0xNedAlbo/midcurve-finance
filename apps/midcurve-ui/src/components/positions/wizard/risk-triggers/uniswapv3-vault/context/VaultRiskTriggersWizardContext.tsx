...
import type { UniswapV3VaultPositionStateResponse } from '@midcurve/api-shared';
...
export interface VaultRiskTriggersWizardState {
...
function computeSteps(): WizardStep[] {
  return [
...
const initialState: VaultRiskTriggersWizardState = {
  currentStepIndex: 0,
  position: null,
...
function wizardReducer(
  state: VaultRiskTriggersWizardState,
  action: WizardAction
): VaultRiskTriggersWizardState {
  switch (action.type) {
    case 'GO_TO_STEP':
...
/**
 * Derive the user's proportional liquidity from vault state.
 * userLiquidity = vaultLiquidity * sharesBalance / totalSupply
 */
export function computeUserLiquidity(
  ...
  return (liquidity * sharesBalance) / totalSupply;
}
...
export function convertOrdersToTriggerState(
  ...
  const slOrder = orders.find((o) => {
    return (
      ...
      activeStates.includes(o.automationState)
    );
  });
...
interface VaultRiskTriggersWizardContextValue {
  state: VaultRiskTriggersWizardState;
  steps: WizardStep[];
...
const VaultRiskTriggersWizardContext =
  createContext<VaultRiskTriggersWizardContextValue | null>(null);

// ----- Provider -----

export function VaultRiskTriggersWizardProvider({
  children,
}: {
  children: ReactNode;
}) {
  ...
  const setVaultAddress = useCallback(
    (vaultAddress: string) =>
      dispatch({ type: 'SET_VAULT_ADDRESS', vaultAddress }),
    []
  );
  ...
  const value: VaultRiskTriggersWizardContextValue = {
    state,
    steps,
    currentStep,
    goNext,
  ...
  return (
    <VaultRiskTriggersWizardContext.Provider value={value}>
      {children}
    </VaultRiskTriggersWizardContext.Provider>
  );
}

// ----- Hook -----

export function useVaultRiskTriggersWizard() {
  const context = useContext(VaultRiskTriggersWizardContext);
  if (!context) {
    throw new Error(
      'useVaultRiskTriggersWizard must be used within VaultRiskTriggersWizardProvider'
    );
  }
  return context;
}
