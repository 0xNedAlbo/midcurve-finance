...
export function ConfigureStep() {
  ...
  // Extract position data with vault-specific types
  ...
  const isWrongAccount = !!(
    isConnected &&
    walletAddress &&
    positionState?.isOwnedByUser === false
  );
  ...
  const currentSqrtPriceX96 = useMemo(() => {
    ...
    return positionState?.sqrtPriceX96 ?? '0';
  }, [state.refreshedSqrtPriceX96, state.discoveredPool, positionState]);
  ...
  // Vault-specific: compute user's proportional liquidity
  const userLiquidity = useMemo(() => {
    const vaultLiquidity = BigInt(positionState?.liquidity || '0');
    const sharesBalance = BigInt(positionState?.sharesBalance || '0');
    const totalSupply = BigInt(positionState?.totalSupply || '0');
    if (totalSupply === 0n) return 0n;
    return (vaultLiquidity * sharesBalance) / totalSupply;
  }, [positionState]);

  // Shares to burn based on withdraw percent
  const sharesToBurn = useMemo(() => {
    const sharesBalance = BigInt(positionState?.sharesBalance || '0');
    return (sharesBalance * BigInt(Math.floor(state.withdrawPercent * 100))) / 10000n;
  }, [positionState?.sharesBalance, state.withdrawPercent]);

  // Calculate current position value in quote tokens (based on user liquidity)
  const currentPositionValue = useMemo(() => {
    const sqrtPriceX96 = BigInt(currentSqrtPriceX96 || '0');
    if (userLiquidity === 0n || sqrtPriceX96 === 0n) return 0n;

    try {
      const { token0Amount, token1Amount } = getTokenAmountsFromLiquidity(
        userLiquidity, sqrtPriceX96, tickLower, tickUpper
      );
  ...
  }, [userLiquidity, currentSqrtPriceX96, tickLower, tickUpper, position?.isToken0Quote]);

  // Calculate liquidity to remove (proportional to user's share)
  const liquidityToRemove = useMemo(() => {
    return (userLiquidity * BigInt(Math.floor(state.withdrawPercent * 100))) / 10000n;
  }, [userLiquidity, state.withdrawPercent]);

  // Remaining liquidity after withdrawal
  const remainingLiquidity = useMemo(() => {
    return userLiquidity - liquidityToRemove;
  }, [userLiquidity, liquidityToRemove]);
  ...
  // Extract SL/TP prices and swap configs from active close orders using shared utility
  const closeOrderPrices = useMemo(() => {
    if (!state.activeCloseOrders.length || !state.discoveredPool || !quoteToken) {
      return {
        stopLossPrice: null as bigint | null,
        takeProfitPrice: null as bigint | null,
        slSwapConfig: null as SwapConfig | null,
        tpSwapConfig: null as SwapConfig | null,
      };
    }

    return extractCloseOrderData(
      state.activeCloseOrders,
      position?.isToken0Quote ?? false,
      state.discoveredPool.token0.decimals,
      state.discoveredPool.token1.decimals,
      quoteToken.decimals,
    );
  }, [state.activeCloseOrders, state.discoveredPool, quoteToken, position?.isToken0Quote]);
  ...
  const simulationPosition = useMemo(() => {
    ...
    try {
      ...
      return new CloseOrderSimulationOverlay({
        ...
        stopLossSwapConfig: closeOrderPrices.slSwapConfig ?? undefined,
        takeProfitSwapConfig: closeOrderPrices.tpSwapConfig ?? undefined,
      });
    } catch {
      return null;
    }
  }, [state.discoveredPool, position, remainingLiquidity, isToken0Base, tickLower, tickUpper, closeOrderPrices]);
  ...
  const renderInteractive = () => (
    <div className="space-y-4">
      ...
      <div className="bg-slate-800/50 backdrop-blur-md border border-slate-700/50 rounded-lg p-4 space-y-3">
        ...
        {/* Shares to burn */}
        {state.withdrawPercent > 0 && positionState && (
          <div className="text-xs text-slate-400">
            Shares to burn:{' '}
            <span className="text-white font-medium">
              {formatCompactValue(sharesToBurn, config?.vaultDecimals ?? 18)}
            </span>
            {' / '}
            {formatCompactValue(BigInt(positionState.sharesBalance), config?.vaultDecimals ?? 18)}
          </div>
        )}
      </div>
      ...
      {!isConnected && (
        <EvmWalletConnectionPrompt
          ...
          description="Connect your wallet to withdraw from this vault position"
        />
      )}
      ...
      {isConnected && isWrongAccount && (
        <EvmAccountSwitchPrompt>
          <p className="text-sm text-slate-400">
            You are not the owner of this vault position. Switch to the correct wallet.
          </p>
        </EvmAccountSwitchPrompt>
      )}
        ...
        <div className="h-full flex flex-col items-center justify-center text-slate-500">
          <div className="text-center space-y-2">
            ...
            <p className="text-xs text-slate-500">
              All vault shares will be burned and your proportional liquidity withdrawn
            </p>
          </div>
        </div>
      );
      ...
      return (
        <div className="h-full flex flex-col min-h-0">
          ...
          <p className="text-xs text-slate-400 mt-2 text-center shrink-0">
            <span className="font-semibold">Remaining Risk Profile.</span> Shows your vault share value after the withdrawal.
          </p>
        </div>
      );
    }
