...
interface VaultWithdrawWizardSummaryPanelProps {
...
export function VaultWithdrawWizardSummaryPanel({
  nextDisabled = false,
  onNext,
  showFinish,
  onFinish,
  ...
  takeProfitPrice,
  children,
}: VaultWithdrawWizardSummaryPanelProps) {
  ...
  // Vault-specific typed config and state
  const vaultConfig = position?.config as UniswapV3VaultPositionConfigResponse | undefined;
  ...
  const withdrawInfo = useMemo(() => {
    if (!position || !state.discoveredPool || !baseToken || !quoteToken || !vaultConfig || !vaultState) return null;

    const currentLiquidity = BigInt(vaultState.liquidity || '0');
    ...
    // Proportional liquidity for user's withdrawn shares
    const sharesToBurn = sharesBalance * BigInt(Math.floor(state.withdrawPercent * 100)) / 10000n;
    const liquidityToRemove = (currentLiquidity * sharesToBurn) / totalSupply;

    try {
      const { token0Amount, token1Amount } = getTokenAmountsFromLiquidity(
        liquidityToRemove,
        sqrtPriceX96,
        vaultConfig.tickLower,
        vaultConfig.tickUpper
      );
      ...
      const userLiquidity = (currentLiquidity * sharesBalance) / totalSupply;
  ...
  }, [position, state.discoveredPool, state.refreshedSqrtPriceX96, state.withdrawPercent, baseToken, quoteToken, isToken0Base, vaultConfig, vaultState]);
  ...
  const withdrawalPnl = useMemo(() => {
    ...
    if (currentCostBasis === 0n) return null;

    const proportionalCostBasis = (currentCostBasis * BigInt(Math.floor(state.withdrawPercent * 100))) / 10000n;
  ...
  const sharesInfo = useMemo(() => {
    if (!vaultState || !vaultConfig) return null;

    const sharesBalance = BigInt(vaultState.sharesBalance);
    const sharesToBurn = sharesBalance * BigInt(Math.floor(state.withdrawPercent * 100)) / 10000n;
    const remainingShares = sharesBalance - sharesToBurn;
    const vaultDecimals = vaultConfig.vaultDecimals;
  ...
  }, [vaultState, vaultConfig, state.withdrawPercent]);
  ...
  return (
    <div className="h-full flex flex-col">
      ...
      <div className="flex-1 space-y-4 overflow-auto">
        ...
        {state.withdrawPercent > 0 && withdrawInfo && baseToken && quoteToken && (
          <div className="p-3 bg-slate-700/30 rounded-lg space-y-2.5">
            <p className="text-xs text-slate-400">
              Withdrawal — {state.withdrawPercent}% of position
            </p>
        ...
        {/* Shares Info */}
        {sharesInfo && sharesInfo.sharesToBurn > 0n && (
          <div className="p-3 bg-slate-700/30 rounded-lg space-y-2.5">
            <p className="text-xs text-slate-400">Vault Shares</p>
            <div className="space-y-1.5">
              <div className="flex justify-between items-center text-sm">
                <span className="text-slate-400">Shares to Burn</span>
                <span className="text-white font-medium">
                  {formatCompactValue(sharesInfo.sharesToBurn, sharesInfo.vaultDecimals)}
                </span>
              </div>
              <div className="flex justify-between items-center text-sm">
                <span className="text-slate-400">Remaining Shares</span>
                <span className="text-slate-300">
                  {formatCompactValue(sharesInfo.remainingShares, sharesInfo.vaultDecimals)}
                </span>
              </div>
            </div>
          </div>
