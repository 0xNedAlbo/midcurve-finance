...
export interface VaultWithdrawWizardState {
  // Current step
  currentStepIndex: number;

  // Position (loaded from API via route params)
  position: UniswapV3VaultPositionData | null;
  ...
  // Close orders (fetched via position data)
  ...
  // Quoted amounts (from backend quote endpoint)
  quotedAmounts: { amount0: string; amount1: string } | null;
...
type WizardAction =
  ...
  | { type: 'GO_NEXT' }
  | { type: 'GO_BACK' }
  | { type: 'SET_POSITION'; position: UniswapV3VaultPositionData }
  ...
  | { type: 'SET_QUOTED_AMOUNTS'; amount0: string; amount1: string }
...
const initialState: VaultWithdrawWizardState = {
  currentStepIndex: 0,
  position: null,
...
function wizardReducer(
  state: VaultWithdrawWizardState,
  action: WizardAction
): VaultWithdrawWizardState {
  switch (action.type) {
    case 'GO_TO_STEP':
    ...
    case 'SET_QUOTED_AMOUNTS':
      return {
        ...state,
        quotedAmounts: { amount0: action.amount0, amount1: action.amount1 },
      };
...
interface VaultWithdrawWizardContextValue {
  state: VaultWithdrawWizardState;
  steps: WizardStep[];
  ...
  // Data loading
  setPosition: (position: UniswapV3VaultPositionData) => void;
  ...
  setQuotedAmounts: (amount0: string, amount1: string) => void;
...
const VaultWithdrawWizardContext = createContext<VaultWithdrawWizardContextValue | null>(null);

// ----- Provider -----

export function VaultWithdrawWizardProvider({ children }: { children: ReactNode }) {
  ...
  const setPosition = useCallback(
    (position: UniswapV3VaultPositionData) => dispatch({ type: 'SET_POSITION', position }),
    []
  );
  ...
  const setQuotedAmounts = useCallback(
    (amount0: string, amount1: string) =>
      dispatch({ type: 'SET_QUOTED_AMOUNTS', amount0, amount1 }),
    []
  );
  ...
  const value: VaultWithdrawWizardContextValue = {
    state,
  ...
  return (
    <VaultWithdrawWizardContext.Provider value={value}>
      {children}
    </VaultWithdrawWizardContext.Provider>
  );
}

// ----- Hook -----

export function useVaultWithdrawWizard() {
  const context = useContext(VaultWithdrawWizardContext);
  if (!context) {
    throw new Error(
      'useVaultWithdrawWizard must be used within VaultWithdrawWizardProvider'
    );
  }
  return context;
}
