...
export interface VaultIncreaseDepositWizardState {
  // Current step
  ...
  // Expected shares from vault mint
  expectedShares: string; // bigint as string
...
type WizardAction =
  ...
  | { type: 'SET_EXPECTED_SHARES'; shares: string }
...
const initialState: VaultIncreaseDepositWizardState = {
  currentStepIndex: 0,
  position: null,
...
function wizardReducer(
  state: VaultIncreaseDepositWizardState,
  action: WizardAction
): VaultIncreaseDepositWizardState {
  switch (action.type) {
    case 'GO_TO_STEP':
    ...
    case 'SET_EXPECTED_SHARES':
      return { ...state, expectedShares: action.shares };
...
interface VaultIncreaseDepositWizardContextValue {
  state: VaultIncreaseDepositWizardState;
  steps: WizardStep[];
  ...
  // Expected shares
  setExpectedShares: (shares: string) => void;
...
const VaultIncreaseDepositWizardContext =
  createContext<VaultIncreaseDepositWizardContextValue | null>(null);

// ----- Provider -----

export function VaultIncreaseDepositWizardProvider({
  children,
}: {
  children: ReactNode;
}) {
  ...
  // Expected shares
  const setExpectedShares = useCallback(
    (shares: string) => dispatch({ type: 'SET_EXPECTED_SHARES', shares }),
    []
  );
  ...
  const value: VaultIncreaseDepositWizardContextValue = {
    state,
  ...
  return (
    <VaultIncreaseDepositWizardContext.Provider value={value}>
      {children}
    </VaultIncreaseDepositWizardContext.Provider>
  );
}

// ----- Hook -----

export function useVaultIncreaseDepositWizard() {
  const context = useContext(VaultIncreaseDepositWizardContext);
  if (!context) {
    throw new Error(
      'useVaultIncreaseDepositWizard must be used within VaultIncreaseDepositWizardProvider'
    );
  }
  return context;
}
