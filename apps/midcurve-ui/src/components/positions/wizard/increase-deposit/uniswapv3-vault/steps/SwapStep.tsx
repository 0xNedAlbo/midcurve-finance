...
import type { PoolSearchTokenInfo, SwapToken, UniswapV3VaultPositionConfigResponse, UniswapV3VaultPositionStateResponse } from '@midcurve/api-shared';
...
export function SwapStep() {
  const { state, setStepValid, goNext, setInteractiveZoom } = useVaultIncreaseDepositWizard();
  const { address: walletAddress, isConnected } = useAccount();
  const walletChainId = useChainId();
  const [swapPrefill, setSwapPrefill] = useState<SwapPrefill | null>(null);

  // Extract position data with vault-specific type casts
  ...
  // For vaults: userLiquidity = vaultLiquidity * sharesBalance / totalSupply (proportional)
  // Combined liquidity = userLiquidity + additionalLiquidity
  const simulationPosition = useMemo(() => {
    if (!state.discoveredPool || !position || !positionState) return null;

    const vaultLiquidity = BigInt(positionState.liquidity);
    const sharesBalance = BigInt(positionState.sharesBalance);
    const totalSupply = BigInt(positionState.totalSupply);

    // Calculate proportional user liquidity
    ...
    const combinedLiquidity = userLiquidity + additionalLiquidity;
  ...
  }, [state.discoveredPool, position, positionState, state.additionalLiquidity, tickLower, tickUpper, isToken0Base, closeOrderPrices]);
