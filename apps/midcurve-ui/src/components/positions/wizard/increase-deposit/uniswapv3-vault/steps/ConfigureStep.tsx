...
export function ConfigureStep() {
  ...
  // Calculate expected shares from additional liquidity
  useEffect(() => {
    if (!vaultState || !calculations.liquidity) {
      setExpectedShares('0');
      return;
    }
  ...
  // Calculate user's proportional liquidity in the vault
  // userLiquidity = vaultLiquidity * sharesBalance / totalSupply
  const existingUserLiquidity = useMemo(() => {
    if (!vaultState) return 0n;
    const vaultLiquidity = BigInt(vaultState.liquidity);
  ...
  // Calculate combined cost basis (existing user liquidity + additional)
  const combinedCostBasis = useMemo(() => {
    if (!state.discoveredPool || !position) return 0n;
    const additionalLiquidity = BigInt(calculations.liquidity || '0');
    const combinedLiquidity = existingUserLiquidity + additionalLiquidity;
  ...
  }, [state.discoveredPool, position, calculations.liquidity, existingUserLiquidity, tickLower, tickUpper, isToken0Base]);
  ...
  // Shows COMBINED position (existing user liquidity + additional liquidity)
  ...
  }, [state.discoveredPool, position, calculations.liquidity, existingUserLiquidity, isToken0Base, tickLower, tickUpper, combinedCostBasis, closeOrderPrices]);
