...
interface VaultIncreaseWizardSummaryPanelProps {
  showSkip?: boolean;
  onSkip?: () => void;
  skipLabel?: string;
  nextLabel?: string;
...
export function VaultIncreaseWizardSummaryPanel({
  showSkip,
  onSkip,
  skipLabel = 'Skip',
  nextLabel = 'Next',
  ...
  takeProfitPrice,
  children,
}: VaultIncreaseWizardSummaryPanelProps) {
  ...
  const sharesData = useMemo(() => {
    ...
    const expectedShares = BigInt(state.expectedShares);
    ...
    // Post-mint ownership percentage: (sharesBalance + expectedShares) / (totalSupply + expectedShares)
    const postMintTotal = totalSupply + expectedShares;
    const postMintBalance = sharesBalance + expectedShares;
    const postMintOwnershipPercent =
      postMintTotal > 0n
        ? Number((postMintBalance * 10000n) / postMintTotal) / 100
        : 0;
    ...
    return {
      ...
      postMintOwnershipPercent,
    };
  }, [vaultState, vaultConfig, state.expectedShares]);
  ...
  return (
    <div className="h-full flex flex-col">
      ...
      <div className="flex-1 space-y-4 overflow-auto">
        ...
        {/* Expected Shares */}
        {sharesData && sharesData.expectedShares > 0n && (
          <div className="p-3 bg-slate-700/30 rounded-lg space-y-2.5">
            <p className="text-xs text-slate-400">Expected Shares</p>
            <div className="space-y-1.5">
              <div className="flex justify-between items-center text-sm">
                <span className="text-slate-400">Shares to Mint</span>
                <span className="text-white font-medium">
                  {formatCompactValue(sharesData.expectedShares, sharesData.vaultDecimals)}
                </span>
              </div>
              <div className="flex justify-between items-center text-sm">
                <span className="text-slate-400">Current Ownership</span>
                <span className="text-white font-medium">
                  {sharesData.currentOwnershipPercent.toFixed(2)}%
                </span>
              </div>
              <div className="flex justify-between items-center text-sm">
                <span className="text-slate-400">Post-Mint Ownership</span>
                <span className="text-green-400 font-medium">
                  {sharesData.postMintOwnershipPercent.toFixed(2)}%
                </span>
              </div>
            </div>
          </div>
