/**
 * Uniswap V3 Pool Price Provider — HTTP Polling
 *
 * Reads pool prices via multicall slot0() every 15 seconds and publishes
 * price changes to RabbitMQ for downstream processing.
 *
 * Chosen over WebSocket eth_subscribe for significantly lower Alchemy CU
 * cost. The per-byte WebSocket pricing makes event subscriptions expensive
 * for price monitoring where only the latest price matters.
 */
...
/** Maximum pools per polling batch */
...
/**
 * UniswapV3 pool price polling batch for a single chain.
...
/**
 * Create polling batches for a chain.
...
export function createSubscriptionBatches(
  ...
  for (let i = 0; i < pools.length; i += MAX_POOLS_PER_SUBSCRIPTION) {
    ...
    batches.push(new UniswapV3PoolSubscriptionBatch(chainId, batchIndex, batchPools));
  }
