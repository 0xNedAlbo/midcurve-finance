/**
 * Deploy VaultPositionCloser Diamond
 *
 * Wraps the DeployVaultPositionCloserDiamond.s.sol Forge script and outputs
 * export commands for use with the db:upsert-contract script.
 *
 * Usage:
 *   SWAP_ROUTER=0x... OWNER=0x... CHAIN=arbitrum pnpm deploy:vault-diamond
 *
 * Add --broadcast to actually deploy (default is dry-run):
 *   SWAP_ROUTER=0x... OWNER=0x... CHAIN=arbitrum pnpm deploy:vault-diamond -- --broadcast --verify
 *
...
async function main(): Promise<void> {
  ...
  if (!swapRouter || !owner || !chain) {
    ...
    console.error('');
    console.error('Usage:');
    console.error('  SWAP_ROUTER=0x... OWNER=0x... CHAIN=arbitrum pnpm deploy:vault-diamond');
    console.error('');
    console.error('Add extra forge flags after --:');
    console.error('  SWAP_ROUTER=0x... OWNER=0x... CHAIN=arbitrum pnpm deploy:vault-diamond -- --broadcast --verify');
    process.exit(1);
  }
  ...
  console.log('=== Deploy VaultPositionCloser Diamond ===');
  ...
  const forgeArgs = [
    'script',
    'script/DeployVaultPositionCloserDiamond.s.sol',
    '--sig',
    'run(address,address)',
    swapRouter,
    owner,
  ...
  const match = output.match(/VaultPositionCloser Diamond:\s*(0x[0-9a-fA-F]{40})/);
