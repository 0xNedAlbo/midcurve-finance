...
class AutomationWalletServiceImpl {
   ...
   *
   * Creates the nonce record if it doesn't exist.
   */
  async getAndIncrementNonce(walletId: string, chainId: number): Promise<number> {
    const nonceHash = `${walletId}/evm/${chainId}`;

    // Use transaction for atomicity
    const result = await this.prisma.$transaction(async (tx) => {
      // Try to find existing nonce record
      let nonceRecord = await tx.automationWalletNonce.findUnique({
        where: { nonceHash },
      });

      if (!nonceRecord) {
        // Create new nonce record starting at 0
        nonceRecord = await tx.automationWalletNonce.create({
          data: {
            walletId,
            nonceHash,
            config: { chainId } as unknown as Prisma.InputJsonValue,
            state: { nonce: 0 } as unknown as Prisma.InputJsonValue,
          },
        });

        this.logger.debug({ walletId, chainId, nonce: 0 }, 'Created nonce record');
        return 0;
      }

      // Get current nonce and increment
      const state = nonceRecord.state as { nonce: number };
      const currentNonce = state.nonce;

      await tx.automationWalletNonce.update({
        where: { nonceHash },
        data: {
          state: { nonce: currentNonce + 1 } as unknown as Prisma.InputJsonValue,
        },
      });

      this.logger.debug({ walletId, chainId, nonce: currentNonce }, 'Incremented nonce');
      return currentNonce;
    });
   ...
   *
   * Used to recover from failed transactions or sync on startup.
   */
  async syncNonce(walletId: string, chainId: number, onChainNonce: number): Promise<void> {
    const nonceHash = `${walletId}/evm/${chainId}`;

    await this.prisma.automationWalletNonce.upsert({
      where: { nonceHash },
      create: {
        walletId,
        nonceHash,
        config: { chainId } as unknown as Prisma.InputJsonValue,
        state: { nonce: onChainNonce } as unknown as Prisma.InputJsonValue,
      },
      update: {
        state: { nonce: onChainNonce } as unknown as Prisma.InputJsonValue,
      },
    ...
    this.logger.info({ walletId, chainId, nonce: onChainNonce }, 'Synced nonce with on-chain value');
  }
