...
/**
 * @title BaseFunding
 * @notice Base contract providing IFunding implementation with hook-based extensibility
 * @dev Strategies that need funding capabilities should inherit from this contract.
 *      All IFunding methods are implemented with standard logic; override hooks for custom behavior.
 *
 * Usage:
 * ```solidity
 * contract MyStrategy is BaseFunding {
 ...
 */
abstract contract BaseFunding is BaseStrategy, CoreControlled, IFunding {
    ...
    function withdrawErc20(
        ...
        address token,
        uint256 amount
    ) external override onlyOwner onlyRunning returns (bytes32 requestId) {
    ...
    function withdrawEth(
        ...
        // Emit event for Core to process
        FundingLib.emitEthWithdrawRequested(requestId, chainId, amount, owner);
    }
