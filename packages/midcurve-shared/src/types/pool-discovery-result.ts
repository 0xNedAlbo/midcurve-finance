 ...
 *
 * Types for pool discovery results across all protocols.
 * Protocol-agnostic design supports Uniswap V3, Orca, Raydium, and future protocols.
 */

import type { PoolInterface, Protocol, UniswapV3Pool } from './pool/index.js';

/**
 * Maps protocol identifier to concrete pool class type.
 * Used for type-safe pool discovery results.
 */
type PoolByProtocol = {
 ...
 *
 * Contains pool metadata from subgraph/indexer along with the full pool object.
 * Generic type P constrains the protocol field and provides type-safe pool access.
 *
 * @template P - Protocol identifier ('uniswapv3', etc.)
 *
 * @example
 * ```typescript
 * const result: UniswapV3PoolDiscoveryResult = {
 ...
 *   feesUSD: "1234.56",
 *   pool: uniswapV3Pool  // Type: UniswapV3Pool
 * };
 * ```
 */
export interface PoolDiscoveryResult<P extends Protocol = Protocol> {
  /**
   ...
   *
   * Type-safe: For UniswapV3PoolDiscoveryResult, this is UniswapV3Pool.
   ...
   */
  pool: P extends keyof PoolByProtocol ? PoolByProtocol[P] : PoolInterface;
}

/**
 * Type alias for Uniswap V3 pool discovery result
 *
 * Provides type-safe access to UniswapV3Pool via the pool field.
 */
export type UniswapV3PoolDiscoveryResult = PoolDiscoveryResult<'uniswapv3'>;

// Future protocol aliases (add as protocols are implemented):
