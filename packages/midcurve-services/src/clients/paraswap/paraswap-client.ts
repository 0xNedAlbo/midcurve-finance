/**
 * Paraswap (Velora) Client
 *
 * Calls the public Paraswap API to get swap quotes and build transactions.
...
export type ParaswapSupportedChainId =
  (typeof PARASWAP_SUPPORTED_CHAIN_IDS)[number];

export function isParaswapSupportedChain(
  chainId: number
): chainId is ParaswapSupportedChainId {
...
export async function getParaswapQuote(
  request: ParaswapQuoteRequest
): Promise<ParaswapQuoteResult> {
  const {
    chainId,
    srcToken,
    srcDecimals,
  ...
  const expiresAt = new Date(
    Date.now() + DEFAULT_QUOTE_VALIDITY_SECONDS * 1000
  ).toISOString();
...
export async function buildParaswapTransaction(
  ...
  const deadline =
    Math.floor(Date.now() / 1000) + DEFAULT_QUOTE_VALIDITY_SECONDS;
