...
- `RefuelOperatorRule` — swaps accrued `MidcurveTreasury` fees to ETH and sends them to the operator EOA when its balance falls below the chain's refuel trigger. Registers its schedule **unconditionally**, and looks up treasuries **per run**, so a treasury registered at runtime — which is what the gas readiness gate does — is picked up within two hours rather than at the next restart. It ran the other way until #125: `onStartup()` queried `shared_contracts` once and returned without calling `registerSchedule` on zero rows, so a registered rule with a healthy startup log and no cron behind it looked identical to a working one. Registered, scheduled, and has-ever-fired are three separate questions, and each run now answers the third with a structured summary rather than leaving it to be inferred.

  The trigger is per-chain, at twice `readinessThresholdWei` from `config/gas-readiness.ts`. It sits deliberately **above** the readiness gate so the treasury gets first chance and the user is never offered a top-up for gas the environment could have supplied itself. A gas-relative floor keeps a near-empty treasury from buying a transaction that costs more than it delivers.

  Each run also reads the treasury's `operator()` and `admin()` — this is the only periodic observer of either drift, since the readiness gate only looks when someone is registering a close order. An operator mismatch **blocks** the refuel: `refuelOperator` is `onlyAdminOrOperator`, so signing it would revert `NotAdminOrOperator` and burn gas from the one address already below the trigger. An admin mismatch is reported only, since the refuel does not depend on `admin`.
...
**Binding drift, both directions.** Every readiness read compares the registered treasury's `admin()` and `operator()` against this environment's configured values. Both are logged at warn level and neither is surfaced to the user, who has no action available *from there*. `RefuelOperatorRule` checks the same two on every run, which is what makes the detection periodic rather than dependent on someone opening a wizard.

- **Operator drift** still lets executions run — those are paid by the operator EOA directly — but the refuel path stops working: `refuelOperator` is `onlyAdminOrOperator`, so a key that is neither **reverts `NotAdminOrOperator`**. It does not misdirect funds to the stale address; it fails, having spent gas from an operator that is below the refuel trigger by definition of having got that far. The refuel rule therefore refuses to sign on a mismatch rather than discovering it on chain. Rotating the operator key produces it; see the note on key recreation in the close-order path. The repair is `setOperator()` from the explorer.
